...
static constexpr Opcode OPCODE_MAP_ADD_SUB[2] = {
...
struct TShiftImm : public DataProcessing {
  ...
  TShiftImm(gshort_t instruction)
    : DataProcessing(
        ...
        ImmShiftOperand((instruction >> 6) & 0x1F, thumb_reg(instruction, 3), (BitShift) ((instruction >> 11) & 0x3))
      ) {
...
struct TAddSubReg : public DataProcessing {
  ...
  TAddSubReg(gshort_t instruction)
    : DataProcessing(
        instruction,
        OPCODE_MAP_ADD_SUB[bool(OPC_MASK & instruction)],
        true,
...
struct TLiteralPoolLoad : public LoadStoreOffset {
  ...
  TLiteralPoolLoad(gshort_t instruction)
    : LoadStoreOffset(
        instruction, false, true, true, false, true,
        CpuState::INDEX_PC, thumb_reg(instruction, 8),
        LoadStoreOffset::WORD, (gword_t) ((instruction & 0xFF) << 2)) {}
};
...
struct TLoadStoreReg : public LoadStoreOffset {
  ...
  TLoadStoreReg(gshort_t instruction)
    : LoadStoreOffset(
        ...
        thumb_reg(instruction, 3), thumb_reg(instruction, 0), (LoadStoreOffset::DataType) ((instruction & B_MASK) >> 10),
        ImmShiftOperand(0, thumb_reg(instruction, 6), BitShift::LEFT)
      ) { }
};
...
struct TLoadSigned : public LoadStore {
  ...
  TLoadSigned(gshort_t instruction)
    : LoadStore(
        instruction, true, true, false, true, true,
        thumb_reg(instruction, 3), thumb_reg(instruction, 0),
        (instruction & DATA_TYPE_MASK) ? LoadStore::SHORT : LoadStore::BYTE,
...
struct TLoadStoreImm5 : public LoadStoreOffset {
  ...
  TLoadStoreImm5(gshort_t instruction)
    : LoadStoreOffset(
        instruction, false, true, true, false, instruction & LOAD_MASK,
        thumb_reg(instruction, 3), thumb_reg(instruction, 0),
        LoadStoreOffset::WORD, ((instruction >> 6) & 0x1F) * 4
      ) { }
};
...
struct TLoadStoreByteImm5 : public LoadStoreOffset {
  ...
  TLoadStoreByteImm5(gshort_t instruction)
    : LoadStoreOffset(
        instruction, false, true, true, false, instruction & LOAD_MASK,
        thumb_reg(instruction, 3), thumb_reg(instruction, 0),
        LoadStoreOffset::BYTE, (instruction >> 6) & 0x1F
      ) { }
};
...
struct TLoadStoreShort : public LoadStore {
  ...
  TLoadStoreShort(gshort_t instruction)
    : LoadStore(
        instruction, true, true, false, instruction & LOAD_MASK, false,
        thumb_reg(instruction, 3), thumb_reg(instruction, 0),
...
struct TLoadStoreImm8 : public LoadStoreOffset {
  TLoadStoreImm8(gshort_t instruction)
    : LoadStoreOffset(
        instruction, false, true, true, false, instruction & LOAD_MASK,
        CpuState::INDEX_SP, thumb_reg(instruction, 8),
        LoadStoreOffset::WORD, (instruction & 0xFF) * 4
      ) { }
};
...
struct TAddToSP : public DataProcessing {
  TAddToSP(gshort_t instruction)
    : DataProcessing(
        instruction, OPCODE_MAP_ADD_SUB[(instruction >> 7) & 1], false, CpuState::INDEX_SP, CpuState::INDEX_SP, RotateOperand(0xF, instruction & 0x7F)
      ) {}
};
...
struct TAddWithSP : public DataProcessing {
  TAddWithSP(gshort_t instruction)
    : DataProcessing(
        instruction, Opcode::ADD, false, CpuState::INDEX_SP, thumb_reg(instruction, 8), RotateOperand(0xF, instruction & 0xFF)
      ) {}
};
...
struct TPushPopRegisterList : public LoadStoreMultiple {
  ...
  TPushPopRegisterList(gshort_t instruction)
    : LoadStoreMultiple(
        instruction, false, true, false, true, instruction & POP_FLAG, CpuState::INDEX_SP, instruction & 0xFF
      ) {
...
struct TLoadStoreMultiple : public LoadStoreMultiple {
  TLoadStoreMultiple(gshort_t instruction)
    : LoadStoreMultiple(
        instruction, false, true, false, false, instruction & LOAD_MASK, thumb_reg(instruction, 8), instruction & 0xFF
      ) {
...
struct TBranchWithLink : public Ins {
  ...
  TBranchWithLink(gshort_t instruction)
    : Ins(instruction),
      opcode((Opcode) ((instruction >> 11) & MASK_OPCODE)),
